...
int main (int argc, char* argv[]) {
  ...
  hls::stream<ap_uint<16>>            tcp_listen_req("tcp_listen_req");
  hls::stream<bool>                   tcp_listen_rsp("tcp_listen_rsp");
  hls::stream<tcp_notification_pkt>   tcp_notification("tcp_notification");
  hls::stream<tcp_xx_req_pkt>         tcp_rx_req("tcp_rx_req");
  hls::stream<ap_uint<16>>            tcp_rx_rsp("tcp_rx_rsp");
  hls::stream<ap_axiu<512, 0, 0, 0>>  tcp_rx_data("tcp_rx_data");
  hls::stream<tcp_xx_req_pkt>         tcp_tx_req("tcp_tx_req");
  hls::stream<tcp_tx_rsp_pkt>         tcp_tx_rsp("tcp_tx_rsp");
  hls::stream<ap_axiu<512, 0, 0, 0>>  tcp_tx_data("tcp_tx_data");
  ...
  for (int i=0; i<TEST_SIZE; i++) {    
    ...
    tcp_xx_req_pkt tx_req = tcp_tx_req.read();
    if (tx_req.sessionID != mockSessionID) {
      this_test = false;
      std::cerr << "ERROR: [" << i << "][tcp_tx_req.sessionID]" << std::endl;
    }
    if (tx_req.length != (mockBody.size() + mockHeaders.size())) {
      this_test = false;
